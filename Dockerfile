...
RUN mkdir -p /usr/src/starlane &&\
    mkdir -p /usr/src/mesh-portal &&\
    mkdir -p /usr/src/wasm-membrane &&\
    mkdir -p /usr/src/mechtron


WORKDIR /usr/src

COPY starlane starlane
COPY mesh-portal mesh-portal
COPY wasm-membrane wasm-membrane
COPY mechtron mechtron

WORKDIR /usr/src/starlane/rust/starlane
